...
> 🛟 **Coincé ?** La page **Intro** dit quoi faire selon le temps que tu as perdu. Les noms
> propres à cet atelier sont dans le glossaire juste en dessous.
...
| `patrolLockTimer` | Compte à rebours en secondes, géré par le jeu. Toi tu le lis, et tu choisis à quoi tu le compares (étape 2) |
...
Ce diagramme est le plan de tout l’atelier. `updateState` décide **quel mode** — les flèches.
`chooseDirection` décide **comment bouger** dans ce mode — les bulles. Pour que `chooseDirection`
puisse lire le mode, il faut d’abord le faire passer par `infos`.
...
case**, il tremblerait sur place — il garde donc sa direction un moment. Le jeu compte ce moment
dans `patrolLockTimer` : tu le lis **avant** de choisir, le jeu le recharge à 1,5 s **après**.
Tu le verras donc tomber à 0 — c’est là que tu tires une nouvelle direction.

Compte trois quarts d’heure : c’est le gros morceau de l’atelier. Les deux règles de la patrouille :

1. `patrolLockTimer > 0` **et** la case devant est libre → continue dans `currentDirection`
...
> ```
> `local` veut dire « cette variable ne sert qu’ici » — mets-le devant tes listes.
...
**Ta liste se construit-elle ?** `print(#directionsLibres)` juste avant le tirage, puis **F12**.
Si le nombre reste à 0, le problème est dans tes `if`, pas dans le tirage.
...
« Proche » se mesure en **cases**, horizontal plus vertical, jamais en diagonale. Le seuil de cet
atelier est **8 cases**.
...
Orange en permanence ? `print(infos.totalDistance)` au début de `chooseDirection`, puis **F12** :
en te rapprochant, le nombre doit descendre vers 0 en restant **positif**. S’il devient négatif,
il te manque la valeur absolue.
...
Toujours pas ? `print(game.scaredTimer)` au début de `updateState`, puis **F12**. Si le nombre
saute à 8 quand tu en manges une, le problème est dans ton `if`, pas dans le jeu.
...
Un seul de ces nombres suffit à changer l’animal. Même trajet de Pac-Man, deux fantômes opposés :
...
**Garde une trace :** une capture de ton fantôme en action, son nom, et ses trois phrases.
...
4. **Le fantôme de 1980.** Dans le vrai Pac-Man, les fantômes ne regardaient pas la distance : ils
   ...
   compte tes décisions dans une variable à toi, déclarée **en dehors** des
   trois fonctions. Écris-la à l’intérieur et elle repartira de zéro à chaque appel. Puis bascule
